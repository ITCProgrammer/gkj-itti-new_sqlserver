<script type="text/javascript">
  function proses_nokkerp(){
        var nokk = document.getElementById("nokk").value;

        if (nokk == 0) {
            window.location.href='PotongBongkar';
        }else{
            window.location.href='PotongBongkar&'+nokk;
        }
    }
    function proses_project() {
        var nokk    = document.getElementById("nokk").value;
        var no_order = document.getElementById("no_order").value;

        if (nokk == "") {
            swal({
                title: 'Nomor KK ERP tidak boleh kosong',   
                text: 'Klik Ok untuk input data kembali',
                type: 'error'
                });
        }else if (no_order == 0){
            swal({
                title: 'Project tidak boleh kosong',   
                text: 'Klik Ok untuk input data kembali',
                type: 'error'
                });
        } else {
            window.location.href='PotongBongkar&'+nokk+'&'+no_order;
        }
    }
</script>
<?PHP
...
<title>Permintaan Potong Sample dan Bongkaran</title>
...
<form role="form" name="form1" id="form1" action="" enctype="multipart/form-data" method="post">
  <div class="alert alert-warning alert-dismissible">
			<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
				...
				<p>Maksimal Dalam Satu Bon Terdapat <strong>6 Kartu Kerja</strong>. No Kartu Kerja Lama Tidak Bisa Dipakai Pada Program Ini.</p>
	</div>
  <div class="row">
    <div class="col-lg-6">	
      ...
      <div class="card">
        <div class="card-header">
          ...
          <input type="submit" value="Save" name="save" id="save" class="btn btn-primary float-right"/>
				  <button type="button" class="btn btn-success float-left" onClick="window.location.href='PotongBongkar'" name="new"><i class="fa fa-file"></i> New</button>
        </div>
        <!-- /.card-header -->
        <?php 
if($Nokk!=""){		  
        $sql 	= "SELECT A.LOTCODE, B.CODE AS NO_ORDER,B.ORDERPARTNERBRANDCODE,B.LONGDESCRIPTION AS BUYER, B.LEGALNAME1 AS LANGGANAN, E.LONGDESCRIPTION AS JENIS_KAIN,
        B.PO_HEADER AS PO_HEADER,B.PO_LINE AS PO_LINE, C.PRODUCTIONDEMANDCODE AS NO_LOT,D.WARNA FROM BALANCE A 
    LEFT JOIN 
    ...
    LEFT JOIN ( 
  	...
  	) D ON A.ITEMTYPECODE = D.ITEMTYPECODE AND 
    A.DECOSUBCODE01 = D.SUBCODE01 AND
    A.DECOSUBCODE02 = D.SUBCODE02 AND
    A.DECOSUBCODE03 = D.SUBCODE03 AND
    A.DECOSUBCODE04 = D.SUBCODE04 AND
    A.DECOSUBCODE05 = D.SUBCODE05 AND
    A.DECOSUBCODE06 = D.SUBCODE06 AND
    A.DECOSUBCODE07 = D.SUBCODE07 AND
    A.DECOSUBCODE08 = D.SUBCODE08 AND
    A.DECOSUBCODE09 = D.SUBCODE09 AND
    A.DECOSUBCODE10 = D.SUBCODE10 
    LEFT JOIN (
    ...
    ) E ON A.ITEMTYPECODE = E.ITEMTYPECODE AND 
    A.DECOSUBCODE01 = E.SUBCODE01 AND
    A.DECOSUBCODE02 = E.SUBCODE02 AND
    A.DECOSUBCODE03 = E.SUBCODE03 AND
    A.DECOSUBCODE04 = E.SUBCODE04 AND
    A.DECOSUBCODE05 = E.SUBCODE05 AND
    A.DECOSUBCODE06 = E.SUBCODE06 AND
    A.DECOSUBCODE07 = E.SUBCODE07 AND
    A.DECOSUBCODE08 = E.SUBCODE08 AND
    A.DECOSUBCODE09 = E.SUBCODE09 AND
    A.DECOSUBCODE10 = E.SUBCODE10 
    WHERE TRIM(A.LOTCODE)='$Nokk'
    LIMIT 1";
          $stmt=db2_exec($conn1,$sql, array('cursor'=>DB2_SCROLLABLE));
          $rowdb2 = db2_fetch_assoc($stmt);
	$sqlBL0 	= "SELECT
	A.LOTCODE, A.PROJECTCODE, B.LONGDESCRIPTION AS BUYER, B.LEGALNAME1 AS LANGGANAN,B.PO_HEADER AS PO_HEADER,B.PO_LINE AS PO_LINE, B.CODE AS NO_ORDER
FROM
	BALANCE A
LEFT OUTER JOIN 
        (SELECT SALESORDER.CODE,SALESORDER.ORDERPARTNERBRANDCODE,ORDERPARTNERBRAND.LONGDESCRIPTION,BUSINESSPARTNER.LEGALNAME1,
...
WHERE
	A.LOTCODE = '$Nokk'
	AND A.LOGICALWAREHOUSECODE = 'M031'
	AND NOT (A.WHSLOCATIONWAREHOUSEZONECODE = 'B1'
		OR A.WHSLOCATIONWAREHOUSEZONECODE = 'TMP')
GROUP BY A.LOTCODE, A.PROJECTCODE, B.LONGDESCRIPTION, B.LEGALNAME1, B.PO_HEADER, B.PO_LINE, B.CODE";
    $stmt0=db2_exec($conn1,$sqlBL0, array('cursor'=>DB2_SCROLLABLE));	
	$rowdb20 = db2_fetch_assoc($stmt0);	  
if($rowdb2['LANGGANAN']!=""){
	$pelanggan = $rowdb2['LANGGANAN']."/".$rowdb2['BUYER'];
}else if($rowdb20['LANGGANAN']!=""){
	$pelanggan = $rowdb20['LANGGANAN']."/".$rowdb20['BUYER'];
}else {
	$pelanggan = "";
}
if($rowdb2['PO_HEADER']!="" or $rowdb2['PO_LINE']!=""){		  
if($rowdb2['PO_HEADER']!=""){$NoPO=$rowdb2['PO_HEADER'];}else{$NoPO=$rowdb2['PO_LINE'];}	
}else if($rowdb20['PO_HEADER']!="" or $rowdb20['PO_LINE']!=""){
if($rowdb20['PO_HEADER']!=""){$NoPO=$rowdb20['PO_HEADER'];}else{$NoPO=$rowdb20['PO_LINE'];}	
}
}
if(TRIM($rowdb2['NO_ORDER'])!=""){$order=$rowdb2['NO_ORDER'];}else{ $order=$rowdb20['NO_ORDER'];}		  
				?>
        <div class="card-body">
				  <div class="row">			
				    <div class="col-sm-12">		
              <div class="form-group row">
                <label for="nokk" class="col-md-3">NoKK ERP</label>
                <div class="col-md-5"> 
                    ...
                    <input name="jabatan_buat" type="hidden" class="form-control form-control-sm" id="jabatan_buat" placeholder="" value="<?php echo $_SESSION['jabatanGKJ']; ?>">		
					          <input name="nokk" onChange="proses_nokkerp()" type="text" class="form-control form-control-sm" id="nokk" placeholder="" value="<?php if($Nokk!=""){ echo $Nokk;} ?>" maxlength="25" <?php if($Nokk!=""){ echo "ReadOnly";} ?> required>	
                ...
                <div class="col-md-6">  
                  <input name="langganan" type="text" class="form-control form-control-sm" id="langganan" placeholder="" value="<?php if(TRIM($rowdb2['LOTCODE'])!=''){echo $pelanggan;} ?>" readonly="readonly">
					      </div>
					      <div class="col-md-2">  
                ...
                <div class="col-md-4">  
                  <input name="no_po" type="text" class="form-control form-control-sm" id="no_po" placeholder="" value="<?php echo $NoPO; ?>" readonly="readonly">
					      </div>
					      <div class="col-md-4">  
                  <input name="no_order" type="text" class="form-control form-control-sm" id="no_order" onChange="proses_project()" placeholder="No Order" value="<?php if($_GET['no_order']!=""){echo $_GET['no_order'];}else{echo $order;} ?>" >
					      </div> 
              </div>
				      <div class="form-group row">
                ...
                <label for="ket" class="col-md-3">Keterangan</label>
					      <div class="col-md-3">
					        <select name="kategori" class="form-control form-control-sm select2" id="kategori" required>>
                    ...
                    <option value="Ganti Grade">Ganti Grade</option>
                  </select>
					      </div>	
                <div class="col-md-5">
			  ...
			  <?php if($_SESSION['deptGKJ']=="QCF"){ ?>			
			  <div class="form-group row">
                <label for="penyebab_qc" class="col-md-3">Penyebab QC</label>
                <div class="col-md-3">  
                  <input type="checkbox" name="penyebab_qc" id="penyebab_qc" value="1" >
					      </div>
				<div class="col-md-5">
					        <textarea name="masalah" class="form-control form-control-sm" placeholder="Masalah.."></textarea>
                  
					      </div>  
              </div>			
			  <?php } ?>			
              <div class="form-group row">
                <label for="potong_null" class="col-md-3">Qty Potong 0</label>
                <div class="col-md-8">  
                  <input type="checkbox" name="potong_null" id="potong_null" value="1" >
					      </div>	
              </div>
			  </div>
					</div>	
          ...
          <table id="example5" width="100%" class="table table-sm table-bordered table-hover" style="font-size: 12px;">
            ...
            <?php
             if($_GET['no_order']!=""){
              ...
              WHERE A.LOTCODE='$Nokk' AND A.PROJECTCODE='$_GET[no_order]' AND A.LOGICALWAREHOUSECODE ='M031' AND NOT (A.WHSLOCATIONWAREHOUSEZONECODE='B1' OR A.WHSLOCATIONWAREHOUSEZONECODE='TMP')
          ...
          <table id="example6" width="100%" class="table table-sm table-bordered table-hover" style="font-size: 12px;">
            ...
            while($rowdt=mysqli_fetch_array($sqlc)){
              ...
              <tr align="center">
                ...
                <td align="left"><?php if($rowdt['jns_permintaan']=="Bongkaran"){echo "<span class='badge badge-warning'>".$rowdt['jns_permintaan']."</span>";}else if($rowdt['jns_permintaan']=="Potong Sample"){echo "<span class='badge badge-success'>".$rowdt['jns_permintaan']."</span>";}else if($rowdt['jns_permintaan']=="Potong Pass Qty"){echo "<span class='badge badge-success'>".$rowdt['jns_permintaan']."</span>";}else if($rowdt['jns_permintaan']=="Ganti Grade"){echo "<span class='badge badge-secondary'>".$rowdt['jns_permintaan']."</span>";}else if($rowdt['jns_permintaan']=="Potong Sisa"){echo "<span class='badge badge-info'>".$rowdt['jns_permintaan']."</span>";}?><br><?php echo $rowdt['ket'];?></td>
