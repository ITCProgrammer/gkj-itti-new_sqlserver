...
<body>	
<form role="form" method="post" enctype="multipart/form-data" name="form1" id="form1" action="SerahTerimaSimpan/">
<input type="hidden" value="<?php echo $_SESSION['userGKJ']; ?>" name="user_serah">	
...
<table id="example1" width="100%" class="table table-sm table-striped table-bordered" style="font-size: 14px;">
 <thead class="btn-info">
  ...
  <?php 
$no=1;  
if (is_array($mutasi->tampildata_serah()))
{	
foreach($mutasi->tampildata_serah() as $rowd){	
	$cekserah=$mutasi->cek_serah($rowd['id_stok'],$rowd['no_mutasi']);
	?>
  <tr align="center">
    ...
    <td align="center"><a href="#" class="btn <?php if($cekserah==""){ echo "disabled";} ?>"  onclick="confirm_delete('DelSerah-<?php echo $rowd[id] ?>');"><?php if($rowd['sts']=="Masih di GKJ"){echo "<span class='badge badge-danger'>".$rowd['sts']."</span>";}else{ echo  "<span class='badge badge-success'>".$rowd['sts']."</span>";}?></a></td>
    <td align="center"><input type="checkbox" name="cek[<?php echo $no; ?>]" value="<?php echo $rowd['id']; ?>" <?php if($rowd['sts']!="Masih di GKJ"){ echo "disabled";} ?>/> </td>
    </tr>
	  <?php $no++; } }?>
  </tbody>
</table>
